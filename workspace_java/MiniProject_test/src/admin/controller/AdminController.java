...
//						apply_ranking();
	...
	private void monthly_sign_up_statistics() {
		...
		if (monthly_sign_up != null) {	// 정보가 있으면
			...
			// 퍼센티지 구하기
			double pct_before_3months = Math.round((float)monthly_sign_up.get("before_3months") / (float)monthly_sign_up.get("total") * 1000) / 1000.0 * 100;
			double pct_before_2months = Math.round((float)monthly_sign_up.get("before_2months") / (float)monthly_sign_up.get("total") * 1000) / 1000.0 * 100;
			double pct_before_1months = Math.round((float)monthly_sign_up.get("before_1months") / (float)monthly_sign_up.get("total") * 1000) / 1000.0 * 100;
			double pct_thismonth = Math.round((float)monthly_sign_up.get("this_month") / (float)monthly_sign_up.get("total") * 1000) / 1000.0 * 100;
	...
	private void monthly_resumes_statistics() {
		...
		if (monthly_resume_write != null) {	// 정보가 있으면
			...
			// 퍼센티지 구하기
			double pct_before_3months = Math.round((float)monthly_resume_write.get("before_3months") / (float)monthly_resume_write.get("total") * 1000) / 1000.0 * 100;
			double pct_before_2months = Math.round((float)monthly_resume_write.get("before_2months") / (float)monthly_resume_write.get("total") * 1000) / 1000.0 * 100;
			double pct_before_1months = Math.round((float)monthly_resume_write.get("before_1months") / (float)monthly_resume_write.get("total") * 1000) / 1000.0 * 100;
			double pct_thismonth = Math.round((float)monthly_resume_write.get("this_month") / (float)monthly_resume_write.get("total") * 1000) / 1000.0 * 100;
