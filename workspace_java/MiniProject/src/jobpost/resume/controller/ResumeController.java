package jobpost.resume.controller;
...
public class ResumeController {
	...
	private void update_resume(Scanner sc, MemberDTO member) {
		...
		do {
			System.out.print("▶ 수정할 이력서 번호 : ");	// 숫자만 입력가능, not null
			resume_no = sc.nextLine();
		...
		if(my_resume != null) {
			...
			String res_title = "";
			do {
				System.out.print("▶ 수정할 제목 : ");	// 수정할 제목조건 - 100글자이내로 작성, 엔터 입력시 넘어감
				res_title = sc.nextLine();
				if(res_title.length() > 100) {
					System.out.println(">> [경고] 제목은 100글자 이내로 작성해야합니다!! << \n");
				}
			...
			do {
				System.out.print("▶ 수정할 학력(1.고졸 2.초대졸 3.대졸 4.석박사) : ");	// 수정할 학력조건 - 숫자만 입력가능, 엔터 입력시 넘어감
				fk_edu_code = sc.nextLine();
				...
				switch (fk_edu_code) {
					...
					case "":
						fk_edu_code = String.valueOf(my_resume.getFk_edu_code());
						exit = true;
						break;
			...
			String fk_major_code = "";
			exit = false;
			do {
				System.out.print("▶ 수정할 전공(1.컴퓨터공학 2.경영학 3.교육학) : ");		// 수정할 전공조건 - 숫자만 입력가능, 엔터 입력시 넘어감
				fk_major_code = sc.nextLine();	
				
				switch (fk_major_code) {
				case "1":	// 컴퓨터공학과
					exit = true;
					break;
				...
				case "":
					fk_major_code = String.valueOf(my_resume.getFk_major_code());
					exit = true;
					break;
			...
			String res_career = "";
			do {
				System.out.print("▶ 수정할 경력[년 단위](없으면 엔터를 입력하세요) : "); // 수정할 경력조건 - 숫자만 입력가능, 엔터 입력시 넘어감
				res_career = sc.nextLine();
				
				if(!res_career.isEmpty()) {
					try {
						Integer.parseInt(res_career);
						break;
					} catch(NumberFormatException e) {
						System.out.println(">> [경고] 경력은 숫자로만 입력해야 합니다!! << \n");
					}
					...
					res_career = String.valueOf(my_resume.getRes_career());
					break;
				}
			...
			// 변수 초기화
			exit = false;
			String fk_work_tcode = "";
			
			do {
				System.out.print("▶ 수정할 고용형태(1.정규직 2.비정규직) : ");	// 수정할 고용형태조건 - 숫자만 입력가능, 엔터 입력시 넘어감
				fk_work_tcode = sc.nextLine();
				
				switch (fk_work_tcode) {
				case "1":	// 정규직
					exit = true;
					break;
				...
				case "":
					fk_work_tcode = String.valueOf(my_resume.getFk_work_tcode());
					exit = true;
					break;
				default:
					System.out.println(">> [경고] 고용형태는 [1, 2] 중에 입력하세요!! << \n");
					break;
				}// end of switch(fk_work_tcode) ------------------
			} while(!exit);
			// end of do~while(!exit)--------------------
			...
			String res_hwage = "";
			do {
				System.out.print("▶ 수정할 희망연봉[만원 단위] : ");	// 수정할 희망연봉조건 - 숫자만 입력가능, 엔터 입력시 넘어감
				res_hwage = sc.nextLine();
				if(!res_hwage.isEmpty()) {
					try {
						Integer.parseInt(res_hwage);
						break;
					} catch (NumberFormatException e) {
						System.out.println(">> [경고] 희망연봉은 숫자로만 입력해야 합니다!! << \n");
					}
					...
					res_hwage = String.valueOf(my_resume.getRes_hwage());
					break;
				}
			} while(true);
			...
			String res_intro = "";
			do {
				System.out.print("▶ 수정할 자기소개 : ");	// 수정할 자기소개조건 - 200글자 이내로 작성, 엔터 입력시 넘어감
				res_intro = sc.nextLine();
				if(res_intro.length() > 200) {
					System.out.println(">> [경고] 자기소개는 200글자 이내로 작성해야합니다!! << \n");
				}
			...
			do {
	        	System.out.print(">> 글을 수정하시겠습니까? [Y/N]: ");
	        	yn = sc.nextLine();
	        	...
	        	if ("y".equalsIgnoreCase(yn)) {
	        		...
	        		paramap.put("fk_edu_code", fk_edu_code);
	        		paramap.put("fk_major_code", fk_major_code);
	        		paramap.put("res_career", res_career);
	        		paramap.put("fk_work_tcode", fk_work_tcode);
	        		paramap.put("res_hwage", res_hwage);
	        		...
	        		if (n == 1) {
	        			...
	        			System.out.println(">> SQL 구문 오류 발생으로 인해 글 수정을 실패했습니다. <<");
	        		}
	        	}
	        	else if("n".equalsIgnoreCase(yn)) {
			...
			System.out.println(">> [경고] 올바른 이력서 번호를 입력하세요!! << \n");
		}
