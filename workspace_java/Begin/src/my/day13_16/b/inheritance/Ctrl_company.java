...
public class Ctrl_company {
	...
	public void register(Scanner sc, Company[] cp_arr) {
		...
		if (Company.count < cp_arr.length) {
			...
			System.out.println("╰（‵□′）╯ 정원 "+ cp_arr.length + "개가 꽉차서 구직자 회원가입이 불가합니다!!!! <<\n");
		}
	...
	public void cp_menu(Scanner sc, Company login_cp, Company[] cp_arr, Gujikja[] gu_arr, Recruit[] rc_arr, Recruit_Apply[] rcApply_arr) { //구인회사 전용 메뉴
...
/*
   ...
   */
	...
	// 우리회사 정보 수정
	private void update_myInfosc(Scanner sc, Company login_cp) {
	...
	private void view_all_gujikja_info(Gujikja[] gu_arr) {// 모든 구직자 조회
	...
	private void search_gujikja(Scanner sc, Gujikja[] gu_arr) {
		...
		do {
			...
			switch (menuno) {
			case "1":	//성별 검색
				search_gender_gujikja(sc, gu_arr);
				break;

			case "2":	//연령대 검색
				search_age_gujikja(sc, gu_arr);
				break;
	...
	// 구직자 성별 검색
	private void search_gender_gujikja(Scanner sc, Gujikja[] gu_arr) {
	...
	// 연령대 검색
	private void search_age_gujikja(Scanner sc, Gujikja[] gu_arr) {
		if(Gujikja.count == 0) {
			System.out.println("구직자로 가입된 회원이 아무도 없습니다.<<");
		}
		else {
	...
	private void search_ageline_gender_gujikja(Scanner sc, Gujikja[] gu_arr) {
		...
		for (int i = 0; i < Gujikja.count; i++) {
			if (((gu_arr[i].age() / 10 * 10) == age) && (gu_arr[i].gender().equals(input_gender.trim()))) {
				isExist = true;
				sb.append(gu_arr[i].getInfo());
			}
		} // end of for문 ------------------------ (정보 쌓기)---------
	...
	private void view_gujikja_my_recruit(Company login_cp, Recruit_Apply[] rcApply_arr) {
