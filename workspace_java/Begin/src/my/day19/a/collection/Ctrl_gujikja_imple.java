...
//			// end of do~while -----------------------
	...
	public Gujikja_imple login(Scanner sc, Map<String, Gujikja_imple> gu_map) {
		...
		if (gu_map.get(id) != null && gu_map.get(id).getPasswd().equals(passwd)) {
				System.out.println("로그인 성공 ㅎㅎ");
...
//	}// end of public Gujikja login(Scanner sc, CommonMember[] cmber_arr)
	...
	private void search_jobtype_company(Scanner sc, List<Company_imple> cp_list) {
		...
		for (int i = 0; i < cp_list.size(); i++) {
			if (cp_list.get(i).getJob_type().toLowerCase().contains(String.join("", job_type_name.split("[ ]")))) {
				sb.append(cp_list.get(i).getInfo() + "\n");
				is_check = true;
			}
		}
			...
			for (int i = 0; i< rc_list.size(); i++) {
				try {
					...
					if(finish_day.after(now)) {
						sb.append(rc_list.get(i).getRecruit_no() + "\t");
						sb.append(rc_list.get(i).getCp().getName() + "\t");
						sb.append(rc_list.get(i).getCp().getJob_type() + "\t");
						sb.append(new DecimalFormat("#,###").format(rc_list.get(i).getCp().getSeed_money()) + "원\t");
						sb.append(rc_list.get(i).getWork_type() + "\t");
						sb.append(rc_list.get(i).getCnt() + "\t");
						sb.append(rc_list.get(i).getRegister_day().substring(0, 4) + "-"
								+ rc_list.get(i).getRegister_day().substring(4, 6) + "-"
								+ rc_list.get(i).getRegister_day().substring(6) + "\t");
						sb.append(rc_list.get(i).getFinish_day().substring(0, 4) + "-"
								+ rc_list.get(i).getFinish_day().substring(4, 6) + "-"
								+ rc_list.get(i).getFinish_day().substring(6) + "\n");
	...
	// 채용응모하기
	private void input_recruitApply(Scanner sc, Gujikja_imple login_gu, List<Recruit_imple> rc_list, List<Recruit_Apply> rcapply_list) {
		...
		while (true) {
			try {
				...
				for (int i = 0; i < rc_list.size(); i++) {
					if (rc_list.get(i).getRecruit_no() == recruit_number) {
						rc = rc_list.get(i);
						isExist = true;
						break;
					}
		...
		for (int i = 0; i < rcapply_list.size(); i++) {
			if (rcapply_list.get(i).getRc().getRecruit_no() == recruit_number) {
				if (rcapply_list.get(i).getGu().getId().equals(login_gu.getId())) {
	...
	private void view_my_recruitApply(Gujikja_imple login_gu, List<Recruit_Apply> rcapply_list) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		...
		try {
			for (int i = 0; i < rcapply_list.size(); i++) {
				...
				if (rcapply_list.get(i).getGu().getId().equals(login_gu.getId())
						&& sdf.parse(rcapply_list.get(i).getRc().getFinish_day()).after(new Date()))	{// 오류
					
					
					System.out.println(rcapply_list.get(i).getRc().recruit_info());
					System.out.println("지원 동기 :"+ rcapply_list.get(i).getApply_motive());
	...
	private void update_my_recruitApply(Scanner sc, Gujikja_imple login_gu, List<Recruit_Apply> rcapply_list) {
		view_my_recruitApply(login_gu, rcapply_list);
	...
	private void view_detail_one_recruit_info(Scanner sc, List<Recruit_imple> rc_list) {
		...
		try {
			...
			for (int i = 0; i < rc_list.size(); i++) {
				if (sdf.parse(rc_list.get(i).getFinish_day()).after(new Date())
						&& input_number.equals(String.valueOf(rc_list.get(i).getRecruit_no()))) {
					is_Exist= true;
