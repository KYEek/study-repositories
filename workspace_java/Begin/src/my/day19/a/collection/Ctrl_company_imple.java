...
//	}
	...
	public Company_imple login(Scanner sc, Map<String, Company_imple> cp_map) {
		...
		if(cp_map.get(id) != null && cp_map.get(id).getPasswd().equals(passwd)) {
	...
	public void cp_menu(Scanner sc, Company_imple login_cp, List<Gujikja_imple> gu_list, List<Recruit_imple> rc_list, List<Recruit_Apply> rcapply_list) { //구인회사 전용 메뉴
		...
		do {
			...
			case "7": // 우리회사 채용공고 지원자 조회
				view_gujikja_my_recruit(login_cp, rcapply_list);
				break;
			case "8": // 로그아웃
...
/*
   ...
   */
	...
	private void view_all_gujikja_info(List<Gujikja_imple> gu_list) {// 모든 구직자 조회
	...
	// 연령대 검색
	private void search_age_gujikja(Scanner sc, List<Gujikja_imple> gu_list) {
		if(gu_list.size() == 0) {
			System.out.println("구직자로 가입된 회원이 아무도 없습니다.<<");
		}
		else {
			...
			for(int i = 0; i< gu_list.size(); i++) {
					...
					if (((gu_list.get(i).age()) / 10 * 10) == age) {
						isExist = true;
						sb.append(gu_list.get(i).getInfo());
					}
			}// end of for문 ------------------------ (정보 쌓기)---------
	...
	private void search_ageline_gender_gujikja(Scanner sc, List<Gujikja_imple> gu_list) {
		...
		for (int i = 0; i < gu_list.size(); i++) {
				if (((gu_list.get(i).age() / 10 * 10) == age) && (gu_list.get(i).gender().equals(input_gender.trim()))) {
					isExist = true;
					sb.append(gu_list.get(i).getInfo());
				}
		} // end of for문 ------------------------ (정보 쌓기)---------
	...
	private void view_recruit_mycompany(Company_imple login_cp, List<Recruit_imple> rc_list) {
		...
		for(int i = 0; i<Recruit_imple.count; i++) {
			if(rc_list.get(i).getCp().getId().equals(login_cp.getId())) {
	...
	private void view_gujikja_my_recruit(Company_imple login_cp, List<Recruit_Apply> rcapply_list) {
		
		StringBuilder sb = new StringBuilder();
		boolean is_Exist = false;
		for(int i = 0; i<rcapply_list.size(); i++) {
			if(login_cp.getId().equals(rcapply_list.get(i).getRc().getCp().getId())) {
				sb.append(rcapply_list.get(i).getRc().getRecruit_no() + "\t");
				sb.append(rcapply_list.get(i).getRc().getSubject() + "\t\t\t\t");
				sb.append(rcapply_list.get(i).getGu().getName() + "\t");
				sb.append(rcapply_list.get(i).getGu().gender() + "\t");
				sb.append(rcapply_list.get(i).getGu().age() + "\t");
