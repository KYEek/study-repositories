...
public class Ctrl_company {
	...
	public Company login(Scanner sc, CommonMember[] cmber_arr) {
		...
		for(int i = 0; i<Company.count; i++) {
			if(id.equals(cmber_arr[i].getId()) && passwd.equals(cmber_arr[i].getPasswd())) {
				return (Company)cmber_arr[i];
			}
		}
		return null;
	}

	public void cp_menu(Scanner sc, Company login_cp, CommonMember[] cmber_arr, Recruit[] rc_arr, Recruit_Apply[] rcApply_arr) { //구인회사 전용 메뉴
...
/*
   ...
   */
		...
		if(CommonMember.count == 0) {	//구직자가 없는 경우
			...
			for (int i = 0; i<CommonMember.count; i++) {
				if(cmber_arr[i].getType() ==1) {
					...
					if(((Gujikja)cmber_arr[i]).gender().equals(input_gender)) {
						sb.append(cmber_arr[i].getInfo() + "\n");
						isFind = true;
					}
				}
		...
		if(CommonMember.count == 0) {
			...
			for(int i = 0; i< CommonMember.count; i++) {
				if (cmber_arr[i].getType() == 1) {
					...
					if (((((Gujikja)cmber_arr[i]).age()) / 10 * 10) == age) {
						isExist = true;
						sb.append(cmber_arr[i].getInfo());
					}
				}
	...
	private void search_ageline_gender_gujikja(Scanner sc, CommonMember[] cmber_arr) {
		...
		for (int i = 0; i < CommonMember.count; i++) {
			if(cmber_arr[i].getType() == 1) {
				if (((((Gujikja)cmber_arr[i]).age() / 10 * 10) == age) && (((Gujikja)cmber_arr[i]).gender().equals(input_gender.trim()))) {
					isExist = true;
					sb.append(cmber_arr[i].getInfo());
				}
			}
