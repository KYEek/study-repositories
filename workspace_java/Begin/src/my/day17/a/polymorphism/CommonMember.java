...
// >> 미완성 클래스 (== 추상 클래스, abstract class)라고 부름 <<
public abstract class CommonMember {
	// Gujikja 클래스와 Company 클래스에서 공통으로 사용되어지는 field(속성) 생성하기
	
	private int type;				//Gujikja 와  Company 를 구분짓는 용도. type값이 1 이라면 Gujikja 라고 보고, 2면 Company로 본다
	...
	public CommonMember() {
		//System.out.println("== 부모 클래스인 CommonMember 클래스의 기본생성자 호출==");
...
//	}
	...
	public void setId(String id) {
		if (id == null || id.isBlank()) {
			...
			// 그리고 userid 의 길이는 최소 2글자, 최대 10글자로 제한한다.
	...
	public void setType(int type) {
		if (type == 1 || type == 2)
			this.type = type;	
		//Gujikja 와  Company 를 구분짓는 용도. type값이 1 이라면 Gujikja 라고 보고, 2면 Company로 본다
	}
