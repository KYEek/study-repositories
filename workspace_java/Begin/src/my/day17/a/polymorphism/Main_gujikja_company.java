...
public class Main_gujikja_company {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case "3": // 구직자 로그인
				Gujikja login_gu = ctrl_gu.login(sc, cmber_arr);
				if (login_gu != null) {
					System.out.println("--구직자 " + login_gu.getName()+"님 로그인 성공ψ(｀∇´)ψ--");
					
					ctrl_gu.gu_menu(sc,login_gu, cmber_arr, rc_arr, rcApply_arr);//구직자 전용 메뉴
				}
				else {
				...
				break;
			case "4": // 구인회사 로그인
				Company login_cp = ctrl_cp.login(sc, cmber_arr);
				if (login_cp != null) {
					System.out.println("--구인회사 " + login_cp.getName()+"기업 로그인 성공ψ(｀∇´)ψ--");
					
					ctrl_cp.cp_menu(sc,login_cp, cmber_arr, rc_arr, rcApply_arr);//구인회사 전용 메뉴
				}
				else {
