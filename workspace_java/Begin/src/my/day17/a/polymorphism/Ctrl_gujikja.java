	...
	void search_ageLine_gender(Scanner sc, CommonMember[] cmber_arr) {
		if(!is_Find_gujikja(cmber_arr)) {
			...
			for(int i = 0; i <CommonMember.count; i++) {
				...
				Gujikja gu = (cmber_arr[i].getType()== 1)?(Gujikja)cmber_arr[i]:null;
...
//							}
	...
	*/
	...
	public Gujikja login(Scanner sc, CommonMember[] cmber_arr) {
	...
	// == 구직자 전용 메뉴 메소드 생성하기 == //
	public void gu_menu(Scanner sc, Gujikja login_gu, CommonMember[] cmber_arr, Recruit[] rc_arr, Recruit_Apply[] rcApply_arr) {
		String str_menuno;
		do {
	...
	private void search_jobtype_company(Scanner sc, CommonMember[] cmber_arr) {
		...
		for(int i =0; i<CommonMember.count; i++) {
			if(cmber_arr[i].getType() == 2) {
				...
				if(((Company)cmber_arr[i]).getJob_type().toLowerCase().contains(String.join("",job_type_name.split("[ ]") ))) {
					sb.append(cmber_arr[i].getInfo() + "\n");
					is_check = true;
				}
			}
		...
		try {
			...
			for (int i = 0; i < CommonMember.count; i++) {
				if (cmber_arr[i].getType() == 2) {
					...
					if (((Company)cmber_arr[i]).getSeed_money() >= search_seed_money) {
						sb.append(cmber_arr[i].getInfo() + "\n");
						is_check = true;
					}
				}
