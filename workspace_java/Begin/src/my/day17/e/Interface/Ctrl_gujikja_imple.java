		...
		if (CommonMember.count < cmber_arr.length) {
			...
			do {
				...
				for (int i = 0; i < CommonMember.count; i++) {
					if (cmber_arr[i].getType() == 1 && id.equals(cmber_arr[i].getId())) {
...
//			// end of do~while -----------------------
			...
			// end of do~while -----------------------
			gu.setType(1);
			cmber_arr[Gujikja_imple.count++] = gu;
			
			System.out.println("(((o(*ﾟ▽ﾟ*)o)))구직자 회원가입 성공!!!");
		}
		else {	// 지금까지 생성된 구직자 회원수가 gu_arr.length(==>정원) 와 같거나 큰 경우에만 신규회원을 받아들이면 안된다......
			System.out.println("╰（‵□′）╯ 정원 "+ cmber_arr.length + "명이 꽉차서 구직자 회원가입이 불가합니다!!!! <<\n");
		}
	}// end of void register(Scanner sc, CommonMember[] cmber_arr) --------------------------------

	
	// === CommonMEmber[] cmbr_arr 에 구직자가 존재하는지 확인하는 용도에요
	public boolean is_Find_gujikja(CommonMember[] cmber_arr) {
		...
		return result;
	}// end of is_Find_gujikja() ---------------------------------------------------------
	
	
	boolean is_Find_company(CommonMember[] cmber_arr) {
	...
	// 구직자 모두보기
	@Override
	public void view_all_info(CommonMember[] cmber_arr) {
		/*
        -----------------------------------------------------------------------------
	...
	//검색하기
	@Override
	public void search_menu(Scanner sc, CommonMember[] cmber_arr) {
		String str_menuno;
		do {
			System.out.println("\n === 검색메뉴 ===\n"
			...
			switch (str_menuno) {
			case "1":	// 연령대 검색
				search_ageLine(sc, cmber_arr);
				break;
			case "2":	// 성별 검색
				search_gender(sc, cmber_arr);
				break;
			case "3":	// 연령대 및 성별 검색
				search_ageLine_gender(sc, cmber_arr);
				break;
			case "4":	// 메인메뉴로 돌아가기
				break;
	...
	void search_ageLine_gender(Scanner sc, CommonMember[] cmber_arr) {
		if(!is_Find_gujikja(cmber_arr)) {
			...
			for(int i = 0; i <CommonMember.count; i++) {
				...
				Gujikja_imple gu = (cmber_arr[i].getType()== 1)?(Gujikja_imple)cmber_arr[i]:null;
				
				if (gu != null) {
					int ageLine = gu.age() / 10 * 10;
					// 나이/10*10 ==> 연령대
					// 37 ==> 30
					// 36 ==> 30
					// 23 ==> 20
			...
			if(!isFind_ageline) {	// 검색하고자 하는 연령대에 해당하는 구직자가 존재하지 않는 경우
				...
				for(int i= 0; i<CommonMember.count; i++) {
					...
					if(gu!=null) {
						if(Integer.parseInt(str_ageLine) == gu.age()/10*10
								&& input_gender.equals(gu.gender()) ) {
...
//							}
	...
	*/
	// 성별 검색
	void search_gender(Scanner sc, CommonMember[] cmber_arr) {
		if(CommonMember.count == 0) {
			System.out.println("(′д｀ )…彡…彡 구직자로 가입된 회원이 아무도 없습니다. \n");
		}
		else {	//구직자가 있는 경우
			...
			for (int i = 0; i < Gujikja_imple.count; i++) {
				...
				if (gu != null) {
					...
					if (input_gender.trim().equals(gu.gender())) {
	...
	//연령대 검색
	void search_ageLine(Scanner sc, CommonMember[] cmber_arr) {
		if(!is_Find_gujikja(cmber_arr)) {
			System.out.println("(′д｀ )…彡…彡 구직자로 가입된 회원이 아무도 없습니다. \n");
		}
		else { // 구직자가 있는 경우
...
//	public Gujikja_imple login(Scanner sc, CommonMember[] cmber_arr) {
//		
...
//					return (Gujikja_imple)cmber_arr[i];
//				}
//			}
//			
//		}//		end 	of		for문
...
//		
//		return null;
//	}// end of public Gujikja login(Scanner sc, CommonMember[] cmber_arr)
	...
	public void gu_menu(Scanner sc, Gujikja_imple login_gu, CommonMember[] cmber_arr, Recruit_imple[] rc_arr, Recruit_Apply[] rcApply_arr) {
		...
		do {
			...
			case "2": // 내정보 수정
				...
				break;
			case "3": // 모든 구인회사 조회
				view_all_company_info(cmber_arr);
				break;
			case "4": // 구인회사 검색
				search_company(sc, cmber_arr);
				break;
			case "5": // 모든 채용공고 조회
	...
	// 내정보 수정
	private void update_myInfosc(Scanner sc, Gujikja_imple login_gu) {
		view_myInfo(login_gu);
	...
	// 모든 회사 정보 표시
	private void view_all_company_info(CommonMember[] cmber_arr) {
		if(!is_Find_company(cmber_arr)) {
	...
	// 구인 회사 검색하기---------------------------------------------------------
	private void search_company(Scanner sc, CommonMember[] cmber_arr) {
		String menuno;
		do {
			...
			switch (menuno) {
			case "1":	//업종 검색
				search_jobtype_company(sc, cmber_arr);
				break;

			case "2":	//자본금 검색
				search_seedmoney_company(sc, cmber_arr);
				break;
			case "3":	//구직자메뉴로 돌아가기
				break;
	...
	// 업종검색
	private void search_jobtype_company(Scanner sc, CommonMember[] cmber_arr) {
		...
		for(int i =0; i<CommonMember.count; i++) {
			if(cmber_arr[i].getType() == 2) {
				...
				if(((Company_imple)cmber_arr[i]).getJob_type().toLowerCase().contains(String.join("",job_type_name.split("[ ]") ))) {
					sb.append(cmber_arr[i].getInfo() + "\n");
					is_check = true;
				}
			}
	...
	//자본금 검색
	private void search_seedmoney_company(Scanner sc, CommonMember[] cmber_arr) {
		...
		try {
			...
			for (int i = 0; i < CommonMember.count; i++) {
				if (cmber_arr[i].getType() == 2) {
					...
					if (((Company_imple)cmber_arr[i]).getSeed_money() >= search_seed_money) {
						sb.append(cmber_arr[i].getInfo() + "\n");
						is_check = true;
					}
				}
	...
	// 채용응모하기
	private void input_recruitApply(Scanner sc, Gujikja_imple login_gu, Recruit_imple[] rc_arr, Recruit_Apply[] rcApply_arr) {
	...
	private void view_my_recruitApply(Gujikja_imple login_gu, Recruit_Apply[] rcApply_arr) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
	...
	private void update_my_recruitApply(Scanner sc, Gujikja_imple login_gu, Recruit_Apply[] rcApply_arr) {
		view_my_recruitApply(login_gu, rcApply_arr);
