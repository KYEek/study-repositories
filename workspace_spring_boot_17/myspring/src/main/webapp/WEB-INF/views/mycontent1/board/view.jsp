...
<script type="text/javascript">
  ...
  <%-- === #113. 이전글제목보기, 다음글제목보기를 할때 글조회수 증가를 하기 위한 것이다. --%>
  ...
  function makeCommentPageBar(currentShowPageNo, totalPage) {
	...
	/*
		...
		( 2/10 ) * 5 + 1;
		( 0.2 ) * 5 + 1;
		Math.floor( 0.2 ) * 5 + 1;  // 소수부가 있을시 Math.floor(0.2) 은 0.2 보다 작은 최대의 정수인 0을 나타낸다.
		0 * 5 + 1 
		1
			       
		currentShowPageNo 가 6페이지 이라면 pageNo 는 6 이 되어야 한다.
		((6 - 1)/5) * 5 + 1;
		( 5/5 ) * 5 + 1;
		( 1 ) * 5 + 1;
		Math.floor( 1 ) * 5 + 1;  // 소수부가 없을시 Math.floor(1) 은 그대로 1 이다.
		1 * 5 + 1
		6
			       
		currentShowPageNo 가 10페이지 이라면 pageNo 는 6 이 되어야 한다.
		((10 - 1)/5) * 5 + 1;
		( 9/5 ) * 5 + 1;
		( 1.8 ) * 5 + 1;
		Math.floor( 1.8 ) * 5 + 1;  // 소수부가 있을시 Math.floor(1.8) 은 1.8 보다 작은 최대의 정수인 1을 나타낸다.
		1 * 5 + 1
		6
		...
		11 12 13 14 15 -- 세번째 블럭의 페이지번호 시작값(pageNo)은 11 이다.
				    
		currentShowPageNo         pageNo
		----------------------------------
		   1                      1 = Math.floor((1 - 1)/5) * 5 + 1
		   2                      1 = Math.floor((2 - 1)/5) * 5 + 1
		   3                      1 = Math.floor((3 - 1)/5) * 5 + 1
		   4                      1 = Math.floor((4 - 1)/5) * 5 + 1
		   5                      1 = Math.floor((5 - 1)/5) * 5 + 1
		   
		   6                      6 = Math.floor((6 - 1)/5) * 5 + 1
		   7                      6 
		   8                      6
		   9                      6
		   10                     6 = Math.floor((10 - 1)/5) * 5 + 1
				        
		   11                    11 = Math.floor((11 - 1)/5) * 5 + 1
		   12                    11 = Math.floor((12 - 1)/5) * 5 + 1
		   13                    11 = Math.floor((13 - 1)/5) * 5 + 1
	*/
...
<%-- === #111. 이전글제목 보기, 다음글제목 보기시 POST 방식으로 넘기기 위한것 === --%>
<form name="goViewFrm">
