...
public class MemberService_imple implements MemberService {
	...
	@Autowired
	private AES256 aes;
	// Type 에 따라 Spring 컨테이너가 알아서 bean 으로 등록된 com.spring.app.common.AES256 의 bean 을 aes 에 주입시켜준다. 
	...
	public MemberVO getLoginMember(Map<String, String> paraMap) {
		...
		if(loginuser != null) {
			...
			if(loginuser.getLastlogingap() >= 12 && loginuser.getIdle() == 0) {
				...
				loginuser.setIdle(1); // 휴면으로 지정
				
				// === tbl_member 테이블의 idle 컬럼의 값을 1로 변경하기 === //
				dao.updateIdle(paraMap.get("userid"));
			}
			
			if(loginuser.getIdle() == 0) { // 휴면이 아닌 경우
			...
			if(loginuser.getPwdchangegap() >= 3) { // 비밀번호가 변경된지 3개월 이상인 경우 
