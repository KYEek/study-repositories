	...
	$("span#emailcheck").click(function(){
		 ...
		 $.ajax({
			 ...
			 success:function(json){
				 ...
				 if(json.isExists) {
					 // 입력한 email 이 현재 사용중이라면
					 $("span#emailCheckResult").html($("input#email").val() + " 은 현재 사용중 이므로 다른 이메일을 입력하세요").css({"color":"red"}); 
					 $("input#email").val(""); 
				 }
				 else {
...
//  또는
	...
	$.ajax({
		...
		data:{"new_pwd":$("input:password[name='pwd']").val()
		     ...
		     }, // data 속성은 http://localhost:9090/MyMVC/member/duplicatePwdCheck.up 로 전송해야할 데이터를 말한다. 
		type:"post", //  type 을 생략하면 type:"get" 이다.
        async:false,  // !!!!! 반드시 동기방식 이어야 한다 !!!!! 
		             // async:true 가 비동기 방식을 말한다. async 을 생략하면 기본값이 비동기 방식인 async:true 이다.
		             // async:false 가 동기 방식이다. 지도를 할때는 반드시 동기방식인 async:false 을 사용해야만 지도가 올바르게 나온다.
        dataType:"json", // Javascript Standard Object Notation.  dataType은 /MyMVC/member/duplicatePwdCheck.up 로 부터 실행되어진 결과물을 받아오는 데이터타입을 말한다. 
		                 // 만약에 dataType:"xml" 으로 해주면 /MyMVC/member/duplicatePwdCheck.up 로 부터 받아오는 결과물은 xml 형식이어야 한다. 
		                 // 만약에 dataType:"json" 으로 해주면 /MyMVC/member/duplicatePwdCheck.up 로 부터 받아오는 결과물은 json 형식이어야 한다. 
        
	    success:function(json){
                // json ==> {"isExists" : true}       또는    {"isExists" : false} 
				//          새암호가 동일한 암호인 경우         새암호가 동일하지 않은 암호인 경우
