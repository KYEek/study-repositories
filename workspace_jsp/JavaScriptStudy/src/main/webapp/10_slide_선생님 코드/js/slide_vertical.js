...
window.onload = function() {
	...
	const image_height = 300; // 한번 이동시 image_height 만큼 이동한다.
    ...
    const func_next = function(){
        if(current_index < max_indexno) {  // 현재 인덱스 번호가 마지막(지금은 3임)이 아닌 경우 
         ...
         // btn_previous.disabled = false; // 이전버튼은 활성화 상태로 만든다.
		    ...
		    positionValue -= image_height; // images 의 위치를 위쪽으로 이동시키도록 image_height 만큼 감소시켜 positionValue에 저장한다. 
            div_images.style.transform = `translateY(${positionValue}px)`; // y축 방향(세로방향)으로 positionValue 만큼 이동하도록 변형시킨다.
