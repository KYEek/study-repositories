$(document).ready(function () {
  ...
  for (let i = 0; i < arrPerson.length; i++) {
    content_html += `<div class="personInfo" style="position: relative; top: 50px; left: ${
      100 + i * 100
    }px;">
  ...
  $("div.personInfo:eq(0)").slideDown(3000);
  $("div.personInfo").eq(1).slideDown(5000);
  $("div.personInfo:eq(2)").slideDown(7000);
  /*
...
*/
  /*   
	   ...
	   숫자 5000 대신에 'normal' 를 쓰면 400 과 같다. 즉, 0.4초 이다.
	   숫자 7000 대신에 'fast'   를 쓰면 200 과 같다. 즉, 0.2초 이다.
	*/
  ...
  $("div.personInfo > ul:nth-child(2) > li:last-child").each(function (
    i,
    elmt
  ) {
    $(elmt).click(function () {
      $("div.personInfo")
        .find("ul#moreProfile" + i)
        .slideDown(3000);
    });
  ...
  $("div.personInfo > ul:last-child > li:last-child").each(function (i, elmt) {
    $(elmt).click(function () {
      // $(elmt).parent().slideUp(3000);
      /*
    		   선택자.slideUp(3000); 은 선택자에 해당되는 내용물을 숫자(단위는 밀리초) 밀리초 동안 아래서 부터 위로 보이지 말라는 말이다.
	           3000은 3초 이다.
    		*/

      $(elmt).parent().slideUp("slow");
      /*   
 		       숫자 3000 대신에 'slow'   를 쓰면 600 과 같다. 즉, 0.6초 이다.
