...
$(document).ready(function () {
  ...
  $("div#container > div#firstdiv > span.buttons").mouseover(function (e) {
     ...
     */
    const filename = arr_person[$(e.target).attr("id")].filename;
    html = `<img src = 'images/${filename}'>`;
    $("div#face").html(html);
    html = `<ol><li><span>이름</span> : ${
      arr_person[$(e.target).attr("id")].name
    }</li>
    <li><span>주소</span> : ${arr_person[$(e.target).attr("id")].address}</li>
    <li><span>이메일</span> : ${
      arr_person[$(e.target).attr("id")].email
    }</li></ol>`;
