...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("button#btnOK").click(function(){
			...
			$.ajax({
			 	url:"<%= ctxPath%>/test/ajax_insert_3.action", 
				type:"post",
	...
	function func_ajax_select(){
		...
		$.ajax({
			url:"<%= ctxPath%>/test/ajax_select_3.action",
			type:"get",
			dataType:"json",
			success:function(json){
				console.log(JSON.stringify(json));
	...
	function func_ajax_select_2(){
		...
		$.ajax({
			url:"<%= ctxPath%>/test/ajax_select_map_one.action",
			type:"get",
			dataType:"json",
			success:function(json){
				console.log(JSON.stringify(json));
...
<body>
    ...
    <p>
        안녕하세요?<br>
        여기는 /test/ajax_form_3.action 페이지 입니다.
    </p>
