...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("button#btnOK").click(function(){
			...
			$.ajax({
			 //	url:"<%= ctxPath%>/test/ajax_insert_2.action", 
				url:"<%= ctxPath%>/test/ajax_insert_2_2.action",
				type:"post",
				
			 //	data:param,
			    data:JSON.stringify(param), /* url 이 ajax_insert_2_2.action 일때 사용함 */ 
			    // data 는 JSON.stringify(param)으로 했으므로 JSON 모양을 띄는 문자열(string)이다.
				
			    contentType:"application/json; charset=utf-8",  
			 /* contentType 은 url 이 ajax_insert_2_2.action 일때 사용함 */
	...
	function func_ajax_select(){
		...
		$.ajax({
			url:"<%= ctxPath%>/test/ajax_select_2.action",
			type:"get",
			dataType:"json",
			success:function(json){
				console.log(JSON.stringify(json));
	...
	function func_ajax_select_2(){
		...
		$.ajax({
			url:"<%= ctxPath%>/test/ajax_select_vo_one.action",
			type:"get",
			dataType:"json",
			success:function(json){
				console.log(JSON.stringify(json));
...
<body>
    ...
    <p>
        안녕하세요?<br>
        여기는 /test/ajax_form_2.action 페이지 입니다.
    </p>
